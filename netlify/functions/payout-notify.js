...
exports.handler = async (event) => {
  try {
    ...
    const { memberId, amount, method, details } = body;
    if (!memberId || !amount) return { statusCode: 400, body: 'missing fields' };
    ...
    if (!AIR_BASE || !AIR_KEY) return { statusCode: 500, body: 'airtable not configured' };
    if (!TELE_TOKEN || !ADMIN_CHAT) return { statusCode: 500, body: 'telegram not configured' };

    const rMember = await fetch(`https://api.airtable.com/v0/${AIR_BASE}/${encodeURIComponent(AIR_TABLE_MEMBERS)}/${memberId}`, {
    ...
    const keyboard = {
      inline_keyboard: [
        [
          { text: 'Approve ✅', callback_data: `approve:${payoutId}` },
          { text: 'Reject ❌', callback_data: `reject:${payoutId}` }
        ]
      ]
    };
